...
void oscEvent(OscMessage msg) {
  ...
  if (msg.checkAddrPattern("server_kick")) {
    //float x = msg.get(0).floatValue();
    scl = 250;
    scl += controlP5.getController("meshScale").getMax()*0.10 ;
    rotationX += 0.2;

    //once scl hits maximum I can't bring it back down to zero

    controlP5.getController("meshScale").setValue(scl);
    if (scl >= controlP5.getController("meshScale").getMax()) {
      scl = 100;
      controlP5.getController("meshScale").setValue(scl);
    }
  ...
  if (msg.checkAddrPattern("count")) {
    float x = msg.get(0).floatValue();
    float y = map(x, 0.0, 1.0, controlP5.getController("meshDistortion").getMin(), controlP5.getController("meshDistortion").getMax());

    float n = noise(x);
    controlP5.getController("meshDistortion").setValue(n);
    //controlP5.getController("randomVCenterRange").setValue(random(controlP5.getController("randomVCenterRange").getMin(),controlP5.getController("randomVCenterRange").getMax()));  
  ...
  if (msg.checkAddrPattern("setup")) {
    ...
    controlP5.getController("form").setValue(12);
    controlP5.getController("meshScale").setValue(100);
    controlP5.getController("meshCount").setValue(450);
    controlP5.getController("meshAlpha").setValue(56);
    controlP5.getController("meshSpecular").setValue(150);
    ranges[0].setHighValue(199.42); // hue
    ranges[0].setLowValue(183.5);
    ranges[1].setHighValue(100.0); //sat
    ranges[1].setLowValue(90.0);
    ranges[2].setHighValue(70.0); // bright
    ranges[2].setLowValue(60.0);
    ranges[3].setHighValue(0.89); //uRange
    ranges[3].setLowValue(-0.92);
    ranges[4].setHighValue(2.49); //vRange
    ranges[4].setLowValue(-2.51);
    controlP5.getController("uCount").setValue(1);
    controlP5.getController("vCount").setValue(30);
    controlP5.getController("randomUCenterRange").setValue(7.93);
    controlP5.getController("paramExtra").setValue(1);
    controlP5.getController("randomScaleRange").setValue(1.0);
    controlP5.getController("meshDistortion").setValue(0.29);
  };
